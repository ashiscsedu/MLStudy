function [sampleLabels,averageL2Error,Aout] = lscc(X,d,K,OPTIONS)
%
%   Linear Spectral Curvature Clustering (LSCC)
%
%   [sampleLabels, averageL2Error] = lscc(X,d,K) 
%   partitions the points in the N-by-D data matrix X into K clusters, 
...
%
%   'sigma' - the tuning parameter contained in the affinity tensor, 
%               can be specified by user when only using a single value; 
...
%
%   'alpha' - number (if >1) or percentage (if <1) of outliers to
...
%                     other algorithms (e.g., k-flats), which is only a
%                     local optima
%
%   (c)2009 Gilad Lerman and Guangliang Chen
%   Last updated on 08/17/2009.
%   Any questions please email glchen@math.umn.edu or lerman@umn.edu
%
